...
# According to the last comment in https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=119544 it's just Acquire::Retries to retry
...
apt-get --yes --allow-change-held-packages -o dpkg::options::=--force-confnew install ${desktop} ${extras} ${tools} || apt-get --yes --fix-broken install
apt-get --yes --allow-change-held-packages -o dpkg::options::=--force-confnew install kali-linux-default --exclude maltego || apt-get --yes --fix-broken install
