...
git clone --depth 1 https://github.com/hardkernel/linux -b odroidw-3.12.y "${basedir}"/kali-${architecture}/usr/src/kernel
...
cp "${basedir}"/../kernel-configs/odroid-w.config .config
cp "${basedir}"/../kernel-configs/odroid-w.config "${basedir}"/kali-${architecture}/usr/src/odroid-w.config
...
dwc_otg.lpm_enable=0 console=ttyAMA0,115200 kgdboc=ttyAMA0,115200 console=tty1 elevator=deadline root=/dev/mmcblk0p2 rootfstype=ext3 rootwait net.ifnames=0 rw
EOF
...
parted ${imagename}.img --script -- mkpart primary ext3 64 -1
