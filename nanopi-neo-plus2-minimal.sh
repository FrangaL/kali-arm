...
# This is Kali Linux ARM image for NanoPi NEO Plus2 (Minimal)
...
imagename=${3:-kali-linux-$1-nanopi-neo-plus2-minimal}
...
current_dir="$(pwd)"
# Base directory
basedir=${current_dir}/nanopi-neo-plus2-"$1"-minimal
# Working directory
...
base="apt-transport-https apt-utils console-setup dialog e2fsprogs ifupdown initramfs-tools iw man-db mlocate netcat-traditional net-tools parted psmisc rfkill screen tmux unrar usbutils vim wget whiptail zerofree firmware-linux firmware-realtek firmware-atheros firmware-libertas kali-defaults snmpd snmp sudo tftp"
# GUI
#desktop="kali-desktop-xfce kali-menu fonts-croscore fonts-crosextra-caladea fonts-crosextra-carlito kali-desktop-xfce kali-root-login lightdm network-manager network-manager-gnome xfce4 xserver-xorg-video-fbdev xfce4-terminal firefox-esr"
...
cp "${current_dir}"/bsp/firmware/veyron/brcm_patchram_plus ${work_dir}/bin/brcm_patchram_plus
chmod 755 "${work_dir}"/bin/brcm_patchram_plus
